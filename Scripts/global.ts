const { world, Button, Card, GameObject, Player, UIElement, Vector } = require('@tabletop-playground/api');
...
const generateUnusedCoords = (gameBoard: gameBoardSpace[][], gameState: typeof state): coord => {
    while (true) {
...
const setUpGameBoard = (gameBoard: gameBoardSpace[][]): void => {
    ...
    if (Math.random() > 0.5) {
        state.firstPlayer = "blue";
    }

    let blueTiles = 8;
    let redTiles = 8;
    if (state.firstPlayer === "blue") {
        blueTiles++;
    } else {
        redTiles++;
    }

    // First, set the assassin
    state.assassinCoords.x = randNumber(0, 5);
    state.assassinCoords.y = randNumber(0, 5);
    gameBoard[state.assassinCoords.x][state.assassinCoords.y].type = "assassin";
...
const makeLabelForClue = (gameBoard: gameBoardSpace[][], gameState: typeof state, loc: coord): typeof GameObject => {
    ...
    return world.createObjectFromTemplate(objGuid, new Vector(gamePos.x, gamePos.y, TABLE_HEIGHT + 1));
}
